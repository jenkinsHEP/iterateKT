...
void plot_deck()
{
    ...
    plotter plotter;
    std::array<double,2> bounds = {0., 4.5};

    double eps = EPS;

    plot p1 = plotter.new_plot();
    p1.set_curve_points(500);
    p1.set_labels("#sigma  [GeV^{2}]",  "#Delta(#it{t}, #it{m}_{3#pi}^{2} #; #sigma)");
    ...
    p1.set_legend(0.35, 0.71);

    plot p2 = plotter.new_plot();
    p2.set_curve_points(500);
    p2.set_labels("#sigma  [GeV^{2}]",  "#Delta(#it{t}, #it{m}_{3#pi}^{2} #; #sigma)");
